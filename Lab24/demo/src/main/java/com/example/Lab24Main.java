...
public class Lab24Main implements Runnable {
	...
	private void setup(int count) {
		...
		for (int i=0; i< numbots; i++) {   // In this loop, for each robot:
            // 1. new up a robot with the shared data structures robots[i] = ???
			// Create a robot and pass the shared queue and stats objects
			robots[i] = new Robot(queue, stats);

			// 2. new up a thread with a robot threads[i] = ???
			// Create a thread for each robot
			threads[i] = new Thread(robots[i]);

			// 3. start the thread thread[i].???
			// Start the thread
			threads[i].start();
		}
	}
	...
	public void run() {
        ...
        // Loop while the counter is positive / there are active robots
        while (counter.get() > 0) { 
            // Create a Parcel
			Parcel parcel = new Parcel();

            // offer / add it to the queue
			queue.offer(parcel);
        ...
        // Loop to join the threads & Wait for all robot threads to finish
